...
func TestLoadDefaults(t *testing.T) {
	...
	assert.Equal(t, 0.42, config.Game.FlySpeed)
	assert.Equal(t, 0.085, config.Game.SwatterRadius)
	assert.Equal(t, 3.5, config.Game.EscapeMultiplier)
	assert.Equal(t, 0.28, config.Game.SwingWindupSeconds)
